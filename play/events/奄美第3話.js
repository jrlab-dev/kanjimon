...
(function(global){
  ...
  function create(host,options={}){
    ...
    function clearActors(){global.AmamiVoice.stop();global.AmamiMotion.stop();qa('.asn-field-enemy').forEach(n=>n.remove());qa('.amami-easy').forEach(n=>n.remove());wordEl=null;easy=null;}
    ...
    function showLine(){global.AmamiVoice.stop();
      ...
      q('#asnText',root).textContent=lines[lineIndex++];box.classList.add('show');global.AmamiVoice.play(q('#asnText',root).textContent,box);
      if(lines[lineIndex-1]==='……もどっても いいよ。'&&easy)global.AmamiMotion.play(easy,'05');
    ...
    function entry(done){
      ...
      later(()=>talk(['ここからは 6年のことば。','……もどっても いいよ。'],done),1850,g);
    ...
    function ending(done){
      ...
      later(()=>talk(['止まるのに、やめない。','……どうして？'],()=>{
        walkHero({x:f.cx-.25,y:f.cy-1.25},1500);
        later(()=>global.AmamiMotion.after(()=>{if(alive(g)&&done)done();}),1620,g);
      }),1200,g);
    }
    function hideForBattle(){clearTimers();clearActors();resetTalk();root.classList.remove('active');document.body.classList.remove('amami-episode3-mode');}
    function destroy(){generation++;clearTimers();clearActors();resetTalk();document.body.classList.remove('amami-episode3-mode');root?.remove();}
